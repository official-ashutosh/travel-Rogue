  ...
  const generateWithAI = async () => {
    ...
    try {
      ...
      const updatedPlan = {
        ...
        top_adventure_activities: recommendationsResponse.data.data.topAdventureActivities,
      ...
      // await api.put(`/api/plans/${planId}`, updatedPlan);
  ...
  const renderSectionContent = (sectionId) => {
    ...
    switch (sectionId) {
      ...
      case 'topplacestovisit':
        return <TopPlacesToVisit places={plan.top_places_to_visit || []} planId={planId} allowEdit={true} />;
      
      case 'itinerary':
        return <Itinerary itinerary={plan.itinerary || []} planId={planId} allowEdit={true} />;
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
        <div className="flex items-center justify-center min-h-screen">
          <div className="text-center space-y-4">
            ...
            <p className="text-gray-600">The plan you're looking for doesn't exist.</p>            <Button onClick={() => navigate('/dashboard')}>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
      ...
      <div className="relative h-96 overflow-hidden">
        ...
        {/* Header Controls */}
        <div className="absolute top-4 left-4 right-4 flex justify-between items-center">          <Button variant="secondary" onClick={() => navigate('/dashboard')}>
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back
          </Button>
        ...
        {planSections.map((section) => (
          activeTab === section.id && (
            <div key={section.id} className="space-y-6">
              ...
              {section.id === 'abouttheplace' && (
                <Card>
                  <CardHeader>
                    <CardTitle>About {plan.nameoftheplace}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      <p className="text-gray-600">
                        Discover interesting facts, history, and culture of your destination.
                      </p>
                      <div className="p-4 bg-green-50 rounded-lg">
                        <p className="text-gray-700">
                          Learn about the rich history, local culture, and unique characteristics that make {plan.nameoftheplace} special...
                        </p>
                      </div>
                    </div>
                  </CardContent>
              ...
              {section.id === 'weather' && (
                <Card>
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <p className="text-gray-600">
                        Weather forecast and climate information for your travel dates.
                      </p>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="p-4 bg-purple-50 rounded-lg text-center">
                          <div className="text-2xl font-bold text-purple-600">5km/h</div>
              ...
              {(section.id === 'adventuresactivitiestodo' || section.id === 'topplacestovisit') && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {[1, 2, 3, 4, 5, 6].map((item) => (
                    <Card key={item} className="hover:shadow-lg transition-shadow">
                      <CardContent className="p-4">
                        <div className="space-y-3">
                          <div className="h-40 bg-gradient-to-br from-blue-400 to-purple-500 rounded-lg"></div>
                          <h3 className="font-medium">
                            {section.id === 'adventuresactivitiestodo' ? `Activity ${item}` : `Place ${item}`}
                          </h3>
                          <p className="text-sm text-gray-600">
                            {section.id === 'adventuresactivitiestodo' 
                              ? 'Exciting adventure awaits you here...' 
                              : 'A must-visit destination in your journey...'}
                          </p>
                          <div className="flex items-center gap-2">
              ...
              {section.id === 'localcuisinerecommendations' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {['Local Specialty 1', 'Famous Restaurant', 'Street Food', 'Traditional Dish', 'Dessert Place', 'Local Market'].map((item, idx) => (
                    <Card key={idx} className="hover:shadow-lg transition-shadow">
                      <CardContent className="p-4">
                        <div className="space-y-3">
                          <div className="h-32 bg-gradient-to-br from-orange-400 to-red-500 rounded-lg flex items-center justify-center">
                            <Utensils className="w-8 h-8 text-white" />
                          </div>
                          <h3 className="font-medium">{item}</h3>
                          <p className="text-sm text-gray-600">
                            A delicious local experience you shouldn't miss...
                          </p>
                          <div className="flex items-center gap-2">
                            <Star className="w-4 h-4 text-yellow-500" />
                            <span className="text-sm">4.7 ({Math.floor(Math.random() * 300) + 50} reviews)</span>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      <div className="space-y-3">
                        <h4 className="font-medium text-blue-600">Clothing</h4>
                        <div className="space-y-2">
                          {['Comfortable walking shoes', 'Weather-appropriate clothes', 'Jacket/Sweater', 'Formal outfit', 'Swimwear'].map((item, idx) => (
                      ...
                      <div className="space-y-3">
                        <h4 className="font-medium text-green-600">Documents</h4>
                        <div className="space-y-2">
                          {['Passport/ID', 'Travel insurance', 'Hotel bookings', 'Flight tickets', 'Emergency contacts'].map((item, idx) => (
                            <label key={idx} className="flex items-center gap-2">
                              <input type="checkbox" className="rounded" />
                              <span className="text-sm">{item}</span>
                            </label>
                      ...
                      <div className="space-y-3">
                        <h4 className="font-medium text-purple-600">Essentials</h4>
                        <div className="space-y-2">
                          {['Phone charger', 'Power bank', 'Medications', 'First aid kit', 'Toiletries'].map((item, idx) => (
                            <label key={idx} className="flex items-center gap-2">
                              <input type="checkbox" className="rounded" />
                              <span className="text-sm">{item}</span>
                            </label>
              ...
              {section.id === 'besttimetovisit' && (
                <Card>
                  <CardHeader>
                    <CardTitle>Best Time to Visit {plan.nameoftheplace}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        {{
                          spring: { months: 'Mar - May', weather: 'Mild & Pleasant', color: 'green' },
                          summer: { months: 'Jun - Aug', weather: 'Warm & Sunny', color: 'yellow' },
                          winter: { months: 'Dec - Feb', weather: 'Cool & Dry', color: 'blue' }
                        }[section.id] && Object.entries({
                          spring: { months: 'Mar - May', weather: 'Mild & Pleasant', color: 'green' },
                          summer: { months: 'Jun - Aug', weather: 'Warm & Sunny', color: 'yellow' },
                          winter: { months: 'Dec - Feb', weather: 'Cool & Dry', color: 'blue' }
                        }).map(([season, details], idx) => (
                          <div key={idx} className={`p-4 bg-${details.color}-50 rounded-lg border border-${details.color}-200`}>
                            <h4 className={`font-medium text-${details.color}-700`}>{season.charAt(0).toUpperCase() + season.slice(1)}</h4>
                            <p className="text-sm text-gray-600">{details.months}</p>
                            <p className="text-sm mt-2">{details.weather}</p>
                          </div>
                        ))}
                      </div>
                      <div className="p-4 bg-blue-50 rounded-lg">
                        <h4 className="font-medium text-blue-700 mb-2">Your Travel Dates</h4>
                        <p className="text-blue-600">
                          {formatDateRange(plan.fromdate, plan.todate)} - Perfect timing for your trip!
                        </p>
                      </div>
                    </div>
                  </CardContent>
