...
import DataTable from "@/frontend/components/expenseTracker/DataTable";
import LoadingComponent from "../../../../app/plans/[planId]/(private)/expense-tracker/loading";
import dynamic from "next/dynamic";
import ExpenseMetrics from "@/frontend/components/expenseTracker/ExpenseMetrics";
...
interface Expense {
  // Define the fields for an expense
  [key: string]: any;
}
...
const ExpenseSection = ({ planId }: { planId: string }) => {
    ...
    const fetchCurrency = async () => {
      try {
        const res = await fetch(
          `/api/plan-settings/preferred-currency?planId=${planId}`
        );
        if (res.ok) {
          const result = await res.json();
          setPreferredCurrency(result.currency || "INR");
        }
      } catch {
  ...
  if (data && data.length === 0)
    return (
      <div className="flex flex-col justify-center items-center gap-2 border-2 border-border rounded-xl w-full h-full shadow-xl font-sans">
