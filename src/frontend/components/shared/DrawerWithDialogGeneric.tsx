...
import { Drawer, DrawerContent, DrawerTrigger } from "@/frontend/components/ui/drawer";
...
function useCurrentUser() {
  ...
  useEffect(() => {
    // TODO: Replace with real REST API call to fetch user info
    const userId =
...
export const GeneratePlanDrawerWithDialog = () => {
  const user = useCurrentUser();
  // Set default credits to 10 if user is null or missing values
  const boughtCredits = user?.credits ?? 10;
  const freeCredits = user?.freeCredits ?? 10;
  const totalCredits = freeCredits + boughtCredits;
  const dialogTriggerBtn = (
    <Button
