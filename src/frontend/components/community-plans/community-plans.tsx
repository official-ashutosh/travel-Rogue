...
import { LoadingPlans } from "@/frontend/components/LoadingPlans";
...
export default function CommunityPlans({
  ...
  const [results, setResults] = useState<CommunityPlan[]>([]);
  const [status, setStatus] = useState<"LoadingFirstPage" | "CanLoadMore" | "Exhausted" | "Idle">("LoadingFirstPage");
  ...
  const fetchPlans = async (append = false) => {
    ...
    try {
      const res = await fetch(`/api/community-plans?companion=${companionId || ""}&offset=${append ? offset : 0}&limit=${limit}`);
      if (res.ok) {
