...
// Define GeminiCompletion type at the top-level so it's available everywhere
...
export const prepareBatch1 = action({
  ...
  handler: async (ctx, { planId }) => {
    try {
      ...
      const geminiCompletion = completion as unknown as GeminiCompletion;
      const geminiText = geminiCompletion?.response?.candidates?.[0]?.content?.parts?.[0]?.text;
      if (!geminiText) throw new Error("No valid response from Gemini API");
      const modelName = JSON.parse(geminiText) as Pick<
        Doc<"plan">,
...
export const prepareBatch2 = action({
  ...
  handler: async (ctx, { planId }) => {
    try {
      ...
      const geminiCompletion2 = completion as unknown as GeminiCompletion;
      const geminiText2 = geminiCompletion2?.response?.candidates?.[0]?.content?.parts?.[0]?.text;
      if (!geminiText2) throw new Error("No valid response from Gemini API");
      const modelName2 = JSON.parse(geminiText2) as Pick<
        Doc<"plan">,
      ...
      await ctx.runMutation(
        ...
        {
          adventuresactivitiestodo: modelName2.adventuresactivitiestodo,
          localcuisinerecommendations: modelName2.localcuisinerecommendations,
          packingchecklist: modelName2.packingchecklist,
          planId: emptyPlan._id,
        }
      );
    } catch (error) {
...
export const prepareBatch3 = action({
  ...
  handler: async (ctx, { planId }) => {
    try {
      ...
      const geminiCompletion3 = completion as unknown as GeminiCompletion;
      const geminiText3 = geminiCompletion3?.response?.candidates?.[0]?.content?.parts?.[0]?.text;
      if (!geminiText3) throw new Error("No valid response from Gemini API");
      const modelName3 = JSON.parse(geminiText3) as Pick<
        Doc<"plan">,
        "itinerary" | "topplacestovisit"
      >;

      await ctx.runMutation(internal.plan.updateItineraryTopPlacesToVisit, {
        itinerary: modelName3.itinerary,
        topplacestovisit: modelName3.topplacestovisit,
        planId: emptyPlan._id,
      });
    } catch (error) {
      throw new ConvexError(
